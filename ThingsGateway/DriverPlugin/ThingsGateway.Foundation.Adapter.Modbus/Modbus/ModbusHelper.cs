...
namespace ThingsGateway.Foundation.Adapter.Modbus
{
    internal class ModbusHelper
    {
        ...
        internal static OperResult<byte[]> GetModbusData(byte[] send, byte[] response)
        {
            try
            {
                ...
                if (send[1] != response[1])
                    return new OperResult<byte[]>(response, "功能码不一致");
                if (response.Length > 3)
                    return OperResult.CreateSuccessResult(GenericHelper.ArrayRemoveBegin(response, 3));
                else
                    return new OperResult<byte[]>(response, "数据长度为0");
            }
            catch (Exception ex)
            {
        ...
        /// 获取读取报文
        /// </summary>
        internal static OperResult<byte[]> GetReadModbusCommand(string address, ushort length, byte station)
        {
            try
            {
        ...
        /// 获取读取报文
        /// </summary>
        private static OperResult<byte[]> GetReadModbusCommand(ModbusAddress mAddress, ushort length)
        {
