	...
	<div class="sys-device-container">
		...
		<el-card shadow="hover" style="margin-top: 8px">
			<el-table :data="deviceData" style="width: 100%" v-loading="loading" border>
				...
				<el-table-column prop="uploadDeviceStatus.deviceOnLineStatus" label="在线状态" width="120" show-overflow-tooltip>
					<template #default="scope">
						<el-tag type="success" v-if="scope.row.uploadDeviceStatus.deviceOnLineStatus == 0">
							{{ DeviceOnLineStatusList(scope.row.uploadDeviceStatus.deviceOnLineStatus) }}
						</el-tag>
						<el-tag type="danger" v-if="scope.row.uploadDeviceStatus.deviceOnLineStatus != 0">
							{{ DeviceOnLineStatusList(scope.row.uploadDeviceStatus.deviceOnLineStatus) }}
						</el-tag>
					</template>
				</el-table-column>
				...
				<el-table-column label="活跃时间" align="center" show-overflow-tooltip>
					<template #default="scope">
						{{ formatDate(new Date(scope.row.uploadDeviceStatus.activeTime), 'mm-dd HH:MM:SS') }}
					</template>
				</el-table-column>
...
import { UploadDeviceRunTimeApi } from '/@/api-services/api';
...
export default defineComponent({
	...
	setup() {
		...
		const handleQuery = async () => {
			state.loading = true;
			var res = await getAPI(UploadDeviceRunTimeApi).apiUploadDeviceRunTimeGetDevicePageGet(
