<template>
	<div class="sys-uploadPlugin-container">
		<el-card shadow="hover" :body-style="{ paddingBottom: '0' }">
			<el-form :model="queryParams" ref="queryForm" :inline="true">
				...
				<el-form-item>
					...
					<el-button icon="ele-Plus" @click="openAdd" v-auth="'uploadPlugin:add'"> 新增 </el-button>
				</el-form-item>
			</el-form>
		</el-card>
				...
				<el-table-column label="操作" width="140" fixed="right" align="center" show-overflow-tooltip>
					<template #default="scope">
						<el-button icon="ele-Delete" size="small" text type="danger" @click="del(scope.row)"
							v-auth="'uploadPlugin:delete'"> 删除 </el-button>
					</template>
				</el-table-column>
		...
		<EditUploadPlugin ref="editUploadPluginRef" :title="edituploadPluginTitle" />
	</div>
</template>
...
import EditUploadPlugin from './component/editUploadPlugin.vue';

import { getAPI } from '/@/utils/axios-utils';
import { UploadPluginApi } from '/@/api-services/api';
import { UploadPlugin } from '/@/api-services/models';

export default defineComponent({
	name: 'uploadPlugin',
	components: { EditUploadPlugin },
	setup() {
		...
		const state = reactive({
			...
			uploadPlugin: [] as Array<UploadPlugin>,
		...
		const handleQuery = async () => {
			...
			state.loading = true;
			var res = await getAPI(UploadPluginApi).apiUploadPluginGetUploadPluginPageGet(state.queryParams.pluginName, undefined, state.tableParams.page, state.tableParams.pageSize);
		...
		// 打开新增页面
		const openAdd = () => {
			state.edituploadPluginTitle = '添加配置';
			editUploadPluginRef.value.openDialog({});
		};
		// 删除
		const del = (row: any) => {
			ElMessageBox.confirm(`确定删除驱动：【${row.name}】? 该操作不会删除驱动文件，后续可再次添加`, '提示', {
				...
				.then(async () => {
					await getAPI(UploadPluginApi).apiUploadPluginDeleteConfigPost({ id: row.id });
					handleQuery();
