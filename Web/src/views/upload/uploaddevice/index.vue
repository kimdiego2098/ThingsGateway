	...
	<div class="sys-device-container">
		...
		<el-card shadow="hover" style="margin-top: 8px">
			<el-table :data="deviceData" style="width: 100%" v-loading="loading" border>
				...
				<el-table-column prop="uploadDevicePropertieNum" label="属性数量" width="120" show-overflow-tooltip />
...
import { UploadDeviceApi} from '/@/api-services/api';
import { UploadDevice } from '/@/api-services/models';
...
export default defineComponent({
	...
	setup() {
		...
		const state = reactive({
			loading: false,
			deviceData: [] as Array<UploadDevice>,
			queryParams: {
				name: undefined,
		...
		const handleQuery = async () => {
			state.loading = true;
			var res = await getAPI(UploadDeviceApi).apiUploadDeviceGetDevicePageGet(
		...
		const delDevice = (row: any) => {
			ElMessageBox.confirm(`确定删除设备：【${row.name}】?`, '提示', {
				...
				.then(async () => {
					await getAPI(UploadDeviceApi).apiUploadDeviceDeleteConfigPost({ id: row.id });
					handleQuery();
		...
		const reStartDevice = (row: any) => {
			...
			ElMessageBox.confirm(mess, '提示', {
				...
				.then(async () => {
					await getAPI(UploadDeviceApi).apiUploadDeviceRestartDevicePost({ id: row.id});
					handleQuery();
		...
		const changeStatus = async (row: any) => {
			await getAPI(UploadDeviceApi)
				.apiUploadDeviceConfigDeviceStatusPost({ id: row.id, invokeEnable: row.invokeEnable })
				.catch(() => {
		...
		const exportDevice = async () => {
			state.loading = true;
			var res = await getAPI(UploadDeviceApi).apiUploadDeviceExportFilePost(state.queryParams, { responseType: 'blob' });
			state.loading = false;
		...
		const importDevice = async () => {
			...
			await getAPI(UploadDeviceApi).apiUploadDeviceImprotFilePostForm(state.fileList[0].raw);
			handleQuery();
