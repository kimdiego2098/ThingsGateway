									...
									<template
										...
										<el-row :gutter="35" v-for="(v, k) in deviceForm.deviceProperties" :key="k">
											<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
												...
												<el-form-item :label="deviceForm.deviceProperties[k].uploadDevicePropertyName"
...
import { UploadDeviceApi, UploadPluginApi } from '/@/api-services/api';
...
export default defineComponent({
	...
	setup() {
		...
		onMounted(async () => {
			...
			let resDicData = await getAPI(UploadPluginApi).apiUploadPluginGetUploadPluginInfosGet();
		...
		const submit = () => {
			deviceFormRef.value.validate(async (valid: boolean) => {
				...
				if (state.deviceForm.id != undefined && state.deviceForm.id > 0) {
					await getAPI(UploadDeviceApi).apiUploadDeviceUpdateConfigPost(state.deviceForm);
				} else {
					await getAPI(UploadDeviceApi).apiUploadDeviceAddDevicePost(state.deviceForm);
				}
				closeDialog();
			});
		};
		...
		const reProperty = async (row: any) => {
			if (JSON.stringify(row) !== '{}') {
				var resRole = await getAPI(UploadDeviceApi).apiUploadDeviceRefreshDevicePropertyPost(row);
